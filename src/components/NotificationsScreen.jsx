...
export default function NotificationsScreen({ userId, onClose }) {
  ...
  const markAllAsRead = async (categoryId) => {
    try {
      ...
      setNotifications(prev =>
        prev.map(n =>
          n.category === categoryId ? { ...n, read: true } : n
        )
      );
  ...
  const getUnreadCount = (categoryId) => {
    return notifications.filter(n => n.category === categoryId && !n.read).length;
  };

  const displayedNotifications = selectedCategory
        ...
        <div className="bg-[#0a0a0a] border border-white/10 rounded-[28px] p-5 mb-8 flex justify-between items-center shadow-lg sticky top-2 z-20">
          ...
          <div className="flex gap-2">
            {!selectedCategory && notifications.length > 0 && (
              <button
                onClick={clearAll}
                className="bg-white/5 hover:bg-white/10 p-4 rounded-[20px] transition-colors active:scale-95"
              >
                ...
                {categories.map(cat => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`bg-gradient-to-br ${cat.bg} border border-${cat.color}-500/40 rounded-[32px] p-6 flex flex-col items-center text-center cursor-pointer relative overflow-hidden aspect-square justify-center shadow-lg`}
              ...
              >
                ...
                <div className="flex justify-between items-center mb-2">
                  {getUnreadCount(selectedCategory) > 0 && (
                    <button
                      onClick={() => markAllAsRead(selectedCategory)}
                      className="flex items-center gap-1.5 text-[10px] font-bold text-white/40 hover:text-white transition-colors bg-white/5 px-4 py-2 rounded-[14px]"
                    >
                      <CheckCircle size={12} />
                      סמן הכל כנקרא
                    </button>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-[10px] font-bold text-white/40 hover:text-rose-400 transition-colors bg-white/5 px-4 py-2 rounded-[14px] mr-auto"
                  >
                    <Trash2 size={12} />
                    נקה הכל
                  </button>
                  ...
                  displayedNotifications.map(notif => {
                    ...
                    return (
                      <motion.div
                        key={notif.id}
                        variants={boxVariants}
                        className={`bg-[#0f0f0f] border rounded-[32px] p-5 flex flex-col relative overflow-hidden transition-all shadow-lg ${
                          !notif.read
                            ? `border-${catColor}-500/50 shadow-[0_0_20px_${categories.find(c => c.id === notif.category)?.hex}20]`
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 pt-1">
                            <h3 className={`font-black text-sm mb-1 ${!notif.read ? 'text-white' : 'text-white/70'}`}>
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {!notif.read && (
                                <Circle size={10} className={`text-${catColor}-500 fill-${catColor}-500`} />
                              )}
                            </div>
                          </div>
                        </div>
