...
import { Zap, Pickaxe, CircleDollarSign, LogOut, Settings, X, ChevronLeft, Link as LinkIcon, Camera, Loader2, Trash2, CheckCircle2, Clock, ShieldCheck, FileText, Info, UserCircle } from 'lucide-react';
...
const UserProfile = ({ user, drops = [] }) => {
  ...
  const handleClaimYield = async () => {
    ...
    try {
      ...
      toast.success(`אספת ${potentialYield} מטבעות!`, { style: { background: '#18181b', color: '#10b981' } });
            ...
            <div className="flex flex-col items-center gap-4">
              <div className="relative">
                <div className="w-32 h-32 rounded-full bg-zinc-800 border-4 border-white/5 overflow-hidden flex items-center justify-center">
                  {uploading ? <Loader2 className="text-white animate-spin" size={32} /> : user.avatar_url ? <img src={user.avatar_url} className="w-full h-full object-cover" alt="פרופיל" /> : <span className="text-4xl text-white/20">@</span>}
                </div>
                <label className="absolute bottom-0 right-0 p-2.5 bg-emerald-500 rounded-full text-white cursor-pointer shadow-xl active:scale-90 transition-all">
                  <Camera size={18} />
      ...
      case 'תקנון שימוש':
        return (
          <div className="text-gray-400 text-sm leading-relaxed space-y-4 animate-in fade-in duration-300 p-2">
            ...
            <p>מערכת ה-Dop Coins היא מערכת וירטואלית בלבד ואין לה ערך כספי מחוץ לאפליקציה.</p>
          </div>
        );
      case 'מדיניות פרטיות':
        return (
      ...
      case 'אודות':
        return (
          <div className="flex flex-col items-center text-center space-y-6 animate-in fade-in duration-300 py-10">
            ...
            <div>
              <h3 className="text-white font-black text-2xl">Dop v1.0.4</h3>
      ...
      default:
        return (
          <div className="space-y-3 animate-in fade-in duration-300">
            {[
              { id: 'עריכת פרופיל', icon: UserCircle },
              { id: 'מדיניות פרטיות', icon: ShieldCheck },
              { id: 'תקנון שימוש', icon: FileText },
              { id: 'אודות', icon: Info }
            ].map((item) => (
              <button key={item.id} onClick={() => setActiveSettingModal(item.id)} className="w-full flex justify-between items-center p-5 bg-zinc-900/50 rounded-[1.5rem] border border-white/5 text-white font-bold active:scale-98 transition-all">
    ...
    <div className="h-[100dvh] w-full bg-[#050505] overflow-y-auto pb-28 hide-scrollbar text-white" dir="rtl">
      ...
      <div className="h-48 bg-gradient-to-b from-zinc-800/40 to-[#050505] relative border-b border-white/5">
        <div className="absolute top-12 left-4 flex gap-3 z-20">
          <button onClick={handleLogout} className="p-3 bg-red-500/10 rounded-2xl text-red-400 border border-red-500/10 active:scale-95 transition-all"><LogOut size={18} /></button>
          <button onClick={() => setShowSettings(true)} className="p-3 bg-white/5 rounded-2xl text-white border border-white/10 active:scale-95 transition-all"><Settings size={18} /></button>
        </div>
      </div>

      <div className="px-4 relative -mt-20 flex flex-col items-center">
        <div className={`w-36 h-36 rounded-full bg-zinc-900 border-[6px] ${user.has_halo ? 'border-emerald-400 shadow-[0_0_50px_rgba(52,211,153,0.3)]' : 'border-black'} overflow-hidden relative z-10 shadow-2xl`}>
        ...
        <h2 className={`mt-5 text-3xl font-black tracking-tighter ${user.has_halo ? 'text-emerald-400' : 'text-white'}`}>
          @{user.username || 'אנונימי'}
        </h2>

        {user.social_link && (
          <a href={user.social_link} target="_blank" rel="noreferrer" className="mt-4 text-emerald-400 font-black text-xs flex items-center gap-2 bg-emerald-500/5 px-6 py-2 rounded-full border border-emerald-500/10 shadow-lg" dir="ltr">
        ...
        <div className="flex gap-3 w-full max-w-sm mt-10">
          <div className="flex-[1.2] bg-zinc-900/50 border border-white/5 rounded-[2.5rem] p-6 flex flex-col items-center shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 inset-x-0 h-20 bg-emerald-500/5 blur-3xl"></div>
            <CircleDollarSign size={24} className="text-emerald-500/50 mb-3" />
          ...
          <div className="flex-1 flex flex-col gap-3">
            ...
            <div className="flex-1 bg-zinc-900/50 border border-white/5 rounded-3xl flex flex-col items-center justify-center py-4">
              <span className="text-2xl font-black text-emerald-500">{totalBoosts}</span>
        ...
        <div className="w-full max-w-sm mt-4">
          <button
            ...
            className={`w-full py-5 rounded-[2rem] font-black text-lg flex items-center justify-center gap-3 transition-all ${canClaim && totalDrops > 0 ? 'bg-gradient-to-r from-emerald-500 to-green-400 text-black shadow-[0_0_30px_rgba(16,185,129,0.3)] active:scale-95' : 'bg-white/5 text-gray-500 border border-white/5 cursor-not-allowed'}`}
          >
            {claiming ? <Loader2 className="animate-spin" size={24} /> : canClaim && totalDrops > 0 ? (
              <><Pickaxe size={24} /> משיכת רווחים</>
            ) : totalDrops === 0 ? (
              <><Pickaxe size={24} /> אין רווחים זמינים</>
            ) : (
              <><CheckCircle2 size={24} className="text-emerald-500" /> הרווחים נאספו להיום</>
            )}
          </button>
      ...
      <div className="mt-14 px-4">
        <div className="flex items-center justify-between mb-6 px-2">
          ...
          <span className="text-[10px] text-emerald-400 font-bold bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20 flex items-center gap-1 shadow-[0_0_10px_rgba(52,211,153,0.1)]">
            ...
            return (
              <div key={drop.id} className="aspect-[3/4] bg-zinc-900 rounded-2xl relative overflow-hidden group shadow-lg border border-white/5">
                ...
                <div className="absolute bottom-2 inset-x-0 text-center flex justify-center">
                   <span className="bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg flex items-center gap-1 text-[9px] font-black text-emerald-400 border border-white/10">
      ...
      {dropToDelete && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-6" onClick={() => setDropToDelete(null)}>
          <div className="bg-zinc-900 border border-white/10 p-8 rounded-[2.5rem] w-full max-w-sm text-center shadow-2xl animate-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
            <h3 className="text-2xl font-black mb-4">למחוק את הדרופ?</h3>
            <p className="text-gray-400 text-sm mb-8 font-bold">מחיקת הדרופ תפסיק את ייצור הרווחים היומי שלו ב-Dop.</p>
            <div className="flex gap-3">
              <button onClick={() => setDropToDelete(null)} className="flex-1 py-4 bg-white/5 rounded-2xl font-bold active:scale-95 transition-all">ביטול</button>
              <button onClick={confirmDelete} className="flex-1 py-4 bg-red-500 text-white font-black rounded-2xl shadow-lg active:scale-95 transition-all">מחק לצמיתות</button>
            </div>
          </div>
        </div>
      )}
