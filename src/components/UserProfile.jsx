...
export default function UserProfile({ user, drops, onClose, onUpdateUser, onDropsUpdate }) {
  ...
  const handleSaveProfile = async () => {
    ...
    setUpdating(true);
    try {
      const updates = { username: usernameInput, profile_link: linkInput };
  ...
  // --- העלאת תמונת פרופיל אמיתית לשרת (תיקיית avatars) ---
  const handleAvatarUpload = async (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    
    const toastId = toast.loading("מעלה תמונת פרופיל...", { style: { background: '#111', color: '#fff' }});
    setUpdating(true);
    ...
    try {
      ...
      toast.success("התמונה עודכנה בהצלחה!", { id: toastId, icon: '📸', style: { background: '#111', color: '#10b981' }});

    } catch (error) {
      console.error(error);
      toast.error("שגיאה בהעלאה. ודא שיש הרשאות Insert לתיקיית avatars", { id: toastId });
    } finally {
      setUpdating(false);
    }
  };
  ...
  const handleSaveDrop = async () => {
    ...
    } catch (error) { toast.error("שגיאה בעריכת הדרופ"); } 
    finally { setUpdating(false); }
  };

  const handleDeleteDrop = async () => {
    if (!editingDrop || !window.confirm("למחוק דרופ זה לצמיתות?")) return;
    setUpdating(true);
    try {
      ...
      setEditingDrop(null);
      if(onDropsUpdate) onDropsUpdate();
    } catch (error) { toast.error("שגיאה במחיקת הדרופ"); } 
    finally { setUpdating(false); }
  };
              ...
              <div className="space-y-4">
                <div className="bg-[#0a0a0a] border border-white/10 rounded-[32px] p-6 shadow-lg text-center flex flex-col items-center">
                  <div className="w-24 h-24 rounded-[24px] bg-[#050505] border-2 border-fuchsia-500/50 mb-4 overflow-hidden flex items-center justify-center relative group">
                    {user.avatar_url ? (
                      <img src={user.avatar_url} alt="Preview" className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={updating} className="mt-2 bg-white/5 border border-white/10 font-bold text-sm py-3 px-6 rounded-[16px] active:scale-95 disabled:opacity-50">בחר מגלריה</button>
                </div>
              ...
              <div className="space-y-4">
                {!editingDrop ? (
                  <div className="space-y-3">
                    <div className="bg-purple-900/20 border border-purple-500/30 p-4 rounded-[20px] mb-4 text-center"><p className="text-purple-300 text-sm font-bold">בחר דרופ לעריכה או מחיקה</p></div>
            ...
            {activeLegalModal && (
              <div className="fixed inset-0 z-[100] bg-[#050505] overflow-y-auto px-4 py-6">
                <div className="flex items-center justify-between mb-8 sticky top-0 bg-[#050505]/90 backdrop-blur-md py-4 z-10">
                  ...
                  <button onClick={() => setActiveLegalModal(null)} className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center active:scale-95"><X size={20} className="text-white" /></button>
                </div>
                <div className="pb-10">
                  {activeLegalModal === 'terms' && (
                     <div className="text-white/80 space-y-6 text-sm leading-relaxed font-medium">
                       <p><strong className="text-emerald-400 text-lg">1. תנאי שימוש כלליים</strong><br/>ברוכים הבאים ל-Dop. השימוש באפליקציה מהווה הסכמה מלאה לתנאים.</p>
                       <p><strong className="text-emerald-400 text-lg">2. כלכלת ה-DOP והיהלומים</strong><br/>המטבעות הוירטואליים מיועדים לשימוש פנימי בלבד (קידום נכסים).</p>
                       <p><strong className="text-emerald-400 text-lg">3. קניין רוחני</strong><br/>כל תוכן המועלה למערכת נשאר בבעלות היוצר.</p>
                     </div>
                  )}
                  {activeLegalModal === 'privacy' && (
                     <div className="text-white/80 space-y-6 text-sm leading-relaxed font-medium">
                       <p><strong className="text-emerald-400 text-lg">1. איסוף נתונים</strong><br/>אנו אוספים נתוני שימוש בסיסיים לשיפור האלגוריתם.</p>
                       <p><strong className="text-emerald-400 text-lg">2. אבטחת מידע</strong><br/>הנתונים שלך נשמרים באמצעות טכנולוגיות ההצפנה המתקדמות ביותר.</p>
                     </div>
                  )}
                  {activeLegalModal === 'about' && (
                     <div className="flex flex-col items-center justify-center pt-20 text-center">
                       <div className="relative w-40 h-40 flex flex-col items-center justify-center mb-8">
                         ...
                         <div className="absolute inset-0 bg-gradient-to-bl from-[#050505] to-[#111] border-2 border-emerald-500/50 rounded-[32px] flex items-center justify-center shadow-2xl z-10"><span className="text-6xl font-black bg-gradient-to-r from-emerald-400 to-teal-200 bg-clip-text text-transparent">DOP</span></div>
                       </div>
                       ...
                       <div className="mt-12 text-[10px] font-bold text-white/30 uppercase tracking-widest">גרסה 1.0.0 (Build 42)</div>
                     </div>
                  )}
                </div>
              </div>
